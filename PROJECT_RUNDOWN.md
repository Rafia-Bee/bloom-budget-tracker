...
-   `idx_expense_user_date_fixed` on (user_id, date, is_fixed_bill) - **NEW 2025-12-17**
-   `idx_expense_user_date` on (user_id, date)
-   `idx_expense_user_category` on (user_id, category)
-   `idx_expense_user_payment` on (user_id, payment_method)
-   Foreign keys indexed for joins with explicit CASCADE/SET NULL behavior
