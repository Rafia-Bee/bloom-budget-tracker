...
-   **Workflows:**
    -   CI/CD (`ci.yml`): Smart path-based testing - only runs tests for changed code
    -   Database Backup (`backup.yml`): Daily at 2 AM UTC (backs up Neon database)
    -   Cleanup (`cleanup.yml`): Repository maintenance

**CI/CD Smart Path Detection:**
| Changed Files | Jobs Run |
|---------------|----------|
| `backend/**` | Backend tests, E2E |
| `frontend/**` | Frontend tests, E2E |
| Both | All tests |
| Docs only | Skip all tests |

**Skip Options:** Add `[skip e2e]` or `[docs only]` to commit message to skip E2E tests
