...
def get_budget_periods():
    ...
    periods = (
        BudgetPeriod.query.filter_by(user_id=current_user_id)
...
def create_budget_period():
    ...
    if period_type == "monthly":
        ...
        if not data.get("start_date") or not data.get("end_date"):
            return (
                jsonify({"error": "Start and end dates required for custom period"}),
                400,
            )
        ...
        return (
            jsonify({"error": "Invalid period type. Use monthly, weekly, or custom"}),
            400,
        )
    ...
    if overlapping:
        return (
            jsonify({"error": "This period overlaps with an existing budget period"}),
            400,
        )
