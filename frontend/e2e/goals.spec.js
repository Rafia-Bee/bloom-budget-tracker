    ...
    test.describe("Create Goal", () => {
        ...
        test("can create a new goal", async ({ page }) => {
            ...
            const nameInput = page
                .locator(
                    'input[name="name"], input[placeholder*="name" i], input'
                )
                .first();
            ...
            // Wait for modal to close OR for the new goal to appear
            // Use Promise.race to handle either outcome
            const modalClosedPromise = page
    ...
    test.describe("Edit Goal", () => {
        test("can access edit functionality for existing goal", async ({
            ...
            if (await editButton.first().isVisible({ timeout: 3000 })) {
                ...
                const goalItem = page.locator(
                    '[data-testid="goal-item"], .goal-item'
                );
                ...
                if (await goalItem.first().isVisible({ timeout: 3000 })) {
                    ...
                    if (await editButton.first().isVisible({ timeout: 3000 })) {
                        ...
                        await expect(
                            page
                                .locator("text=/Edit Goal|Update Goal/i")
                                .first()
                        ).toBeVisible({ timeout: 3000 });
                    }
                }
