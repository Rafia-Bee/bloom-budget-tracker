...
test.describe("Recurring Expenses", () => {
  ...
  test.describe("Create Recurring Template", () => {
    ...
    test("can create a new recurring expense template", async ({ page }) => {
      // Wait for page to load
      await page.waitForSelector("button:has-text('Active')", {
        timeout: 10000,
      });
  ...
  test.describe("Edit Recurring Template", () => {
    ...
    test("edit modal pre-fills existing values", async ({ page }) => {
      ...
      // Wait for modal to open - look for modal container
      ...
      // Name field should have a value - use more flexible selectors
      ...
      if (inputs.length > 0) {
        const nameValue = await inputs[0].inputValue().catch(() => "");
      ...
      if (amountInputs.length > 0) {
        const amountValue = await amountInputs[0].inputValue().catch(() => "");
