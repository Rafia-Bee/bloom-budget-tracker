...
function SalaryPeriodRolloverPrompt({ onCreateNext, onDismiss, salaryPeriodData }) {
    ...
    const {
        daysRemaining,
        isOverdue,
        suggestedDebitBalance,
        suggestedCreditAvailable,
        creditLimit,
    } = rolloverData;

    return (
        <div
            className={`rounded-xl p-6 mb-6 ${isOverdue ? 'bg-red-50 border-2 border-red-300 dark:bg-red-900/20 dark:border-red-700' : 'bg-yellow-50 border-2 border-yellow-300 dark:bg-yellow-900/20 dark:border-yellow-700'}`}
        >
            <div className="flex items-start gap-4">
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                        <svg
                            className={`w-6 h-6 ${isOverdue ? 'text-red-600 dark:text-red-400' : 'text-yellow-600 dark:text-yellow-400'}`}
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                        >
                    ...
                    <div
                        className={`text-sm mb-4 ${isOverdue ? 'text-red-600 dark:text-red-300' : 'text-yellow-600 dark:text-yellow-300'}`}
                    >
                        <p className="font-semibold mb-1">Calculated balances for next period:</p>
                        <ul className="list-disc list-inside space-y-1 ml-2">
                            <li>Debit: {fc(suggestedDebitBalance)}</li>
                            <li>
                                Credit Available: {fc(suggestedCreditAvailable)} / {fc(creditLimit)}
                            </li>
                        </ul>
                    </div>
