...
const WeeklyBudgetCard = forwardRef(
    (
        ...
        useEffect(() => {
            ...
            } else if (selectedPeriod?.salary_period_id) {
                loadSalaryPeriodData(selectedPeriod.salary_period_id);
            } else {
                loadWeeklyData();
            }
        ...
        // Sync selectedWeek when external selectedPeriod changes
        useEffect(() => {
            if (!selectedPeriod || !weeklyData?.salary_period) return;

            // If selected is a sub-period (has week_number)
            if (selectedPeriod.week_number) {
                // Check if the selected period belongs to the currently loaded salary period
                const periodBelongsToLoadedSP =
                    selectedPeriod.salary_period_id === weeklyData.salary_period.id;

                if (periodBelongsToLoadedSP) {
                    setSelectedWeek(selectedPeriod.week_number);
                } else if (selectedPeriod.salary_period_id) {
                    // Selected period belongs to a different salary period - load that one
                    loadSalaryPeriodData(selectedPeriod.salary_period_id);
                }
            }
            // If selected is a salary period (has weekly_budget but no week_number)
            else if (selectedPeriod.weekly_budget !== undefined) {
                // Check if it's a different salary period than currently loaded
                if (selectedPeriod.id !== weeklyData.salary_period.id) {
                    loadSalaryPeriodData(selectedPeriod.id);
                }
            }
            // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        const loadSalaryPeriodData = async (salaryPeriodId) => {
            try {
                setLoading(true);
                const response = await api.get(`/salary-periods/${salaryPeriodId}`);
                setWeeklyData(response.data);
                // Set selected week to the one from selectedPeriod
                if (selectedPeriod?.week_number) {
                    setSelectedWeek(selectedPeriod.week_number);
                } else if (response.data?.current_week?.week_number) {
                    // Only use current_week if it has a valid week_number
                    setSelectedWeek(response.data.current_week.week_number);
                } else if (response.data?.salary_period?.weeks?.length > 0) {
                    // If no current week (past period), select the first week
                    setSelectedWeek(response.data.salary_period.weeks[0].week_number);
                }
                setError(null);
            } catch (err) {
                if (err.response?.status === 404) {
