...
function DebtPayoffChart({ data, currencyFormatter }) {
  ...
  const formatDateLabel = (timestamp) => {
    return new Date(timestamp).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: '2-digit' })
  }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white dark:bg-dark-elevated p-3 rounded-lg shadow-lg border border-gray-200 dark:border-dark-surface">
          ...
          {/* Total Balance */}
          {payload.filter(p => p.name === 'Total Debt').map((entry, index) => (
            <p key="total" className="text-sm font-bold mb-2" style={{ color: entry.color }}>
