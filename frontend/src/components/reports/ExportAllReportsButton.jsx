...
function ExportAllReportsButton({ chartRefs, dateRange, categoryData }) {
    ...
    const exportAllToPNG = async () => {
        ...
        try {
                ...
                for (const category of categoryData.categories) {
                    ...
                    try {
                        ...
                        if (subcategories && subcategories.length > 0) {
                            const canvas = createSubcategoryCanvas(
                                category.name,
                                subcategories,
                                isDarkMode
                            );
            ...
            const maxMainWidth =
                mainCanvases.length > 0 ? Math.max(...mainCanvases.map((c) => c.canvas.width)) : 0;
            const subcategoryWidth =
                subcategoryCanvases.length > 0 ? subcategoryCanvases[0].canvas.width : 0;
