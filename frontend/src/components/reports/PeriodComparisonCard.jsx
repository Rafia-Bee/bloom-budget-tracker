...
function PeriodComparisonCard({ currencyFormatter = formatCurrency }) {
  ...
  const ChangeIndicator = ({ change, changePercent, invertColors = false }) => {
    ...
    if (invertColors) {
      ...
      colorClass = isNegative ? 'text-green-600 dark:text-green-400' :
                   isPositive ? 'text-red-600 dark:text-red-400' :
                   'text-gray-500 dark:text-gray-400'
    } else {
      // For income: increase is good
      colorClass = isPositive ? 'text-green-600 dark:text-green-400' :
    ...
    return (
      <div className={`flex items-center gap-1 text-sm font-medium ${colorClass}`}>
        ...
        {isNeutral ? (
          ...
          <span>
            {currencyFormatter(Math.abs(change))} ({changePercent > 0 ? '+' : ''}{changePercent}%)
          </span>
        )}
      </div>
    )
        ...
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3">
          ...
          <div className="flex flex-wrap gap-2">
            <input
              ...
              onChange={(e) => setPreviousPeriod(prev => ({ ...prev, start: e.target.value }))}
            ...
            <input
              ...
              onChange={(e) => setPreviousPeriod(prev => ({ ...prev, end: e.target.value }))}
              className="px-2 py-1 text-sm rounded border border-purple-200 dark:border-purple-800 bg-white dark:bg-dark-elevated text-gray-900 dark:text-white flex-1 min-w-[120px]"
            />
          </div>
        </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
          ...
          <div className="flex flex-wrap gap-2">
            <input
              ...
              onChange={(e) => setCurrentPeriod(prev => ({ ...prev, start: e.target.value }))}
            ...
            <input
              ...
              onChange={(e) => setCurrentPeriod(prev => ({ ...prev, end: e.target.value }))}
              className="px-2 py-1 text-sm rounded border border-blue-200 dark:border-blue-800 bg-white dark:bg-dark-elevated text-gray-900 dark:text-white flex-1 min-w-[120px]"
            />
          </div>
        </div>
      ...
      {data && !loading && (
        <>
          ...
          <div className="text-center text-sm text-gray-500 dark:text-gray-400 mb-4">
            <span className="text-purple-600 dark:text-purple-400 font-medium">Period B:</span> {formatDateRange(data.previous_period.start, data.previous_period.end)}
            <span className="mx-2">vs</span>
            <span className="text-blue-600 dark:text-blue-400 font-medium">Period A:</span> {formatDateRange(data.current_period.start, data.current_period.end)}
          </div>
                ...
                {data.comparison.category_changes.map((cat) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs">
                        <span className="text-purple-600 dark:text-purple-400">{currencyFormatter(cat.previous)}</span>
                        <span className="text-gray-400"> → </span>
                        <span className="text-blue-600 dark:text-blue-400">{currencyFormatter(cat.current)}</span>
                      </p>
                    </div>
                    <div className="ml-4">
                      <ChangeIndicator
          ...
          {/* No Data State */}
          {data.previous_period.total_spending === 0 && data.previous_period.total_income === 0 &&
           data.current_period.total_spending === 0 && data.current_period.total_income === 0 && (
