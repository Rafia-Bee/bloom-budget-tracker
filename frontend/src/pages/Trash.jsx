                ...
                {deletedRecurring.length === 0 ? (
                  ...
                  deletedRecurring.map(recurring => (
                    <DeletedItemCard
                      ...
                      onRestore={() => handleRestore('recurring', recurring.id)}
  ...
  return (
    <div className="bg-white dark:bg-dark-surface rounded-lg border border-gray-200 dark:border-dark-border p-4">
      <div className="flex items-center justify-between">
        ...
        {amount && (
          <div className={`text-right mr-4 ${isIncome ? 'text-green-600 dark:text-green-400' : 'text-gray-900 dark:text-dark-text'}`}>
            <span className="font-medium">{isIncome ? '+' : '-'}{amount}</span>
          </div>
        )}
        <button
          onClick={onRestore}
