...
describe('AddRecurringExpenseModal', () => {
    ...
    describe('Rendering', () => {
        it('renders the modal with title for add mode', async () => {
            renderWithSharedData(
                <AddRecurringExpenseModal onClose={mockOnClose} onAdd={mockOnAdd} />
            );

            expect(
                screen.getByRole('heading', { name: 'Add Recurring Expense' })
            ).toBeInTheDocument();
        });
    ...
    describe('Debt Integration', () => {
        ...
        it('shows debt names as subcategory options', async () => {
            const mockDebts = [{ id: 1, name: 'Student Loan', monthly_payment: 25000 }];
            debtAPI.getAll.mockResolvedValue({ data: mockDebts });
        ...
        it('autofills name with Payment suffix when debt is selected', async () => {
            const mockDebts = [{ id: 1, name: 'Car Loan', monthly_payment: 30000 }];
            debtAPI.getAll.mockResolvedValue({ data: mockDebts });
    ...
    describe('Goal Integration', () => {
        ...
        it('shows goal names as subcategory options for Savings', async () => {
            ...
            await selectWithAct(
                screen.getByDisplayValue('Fixed Expenses'),
                'Savings & Investments'
            );
