...
def fix_imported_salary_periods():
    ...
    with app.app_context():
        ...
        for sp in salary_periods:
            ...
            print(
                f"    Start: {sp.start_date}, Debit: €{sp.initial_debit_balance/100:.2f}"
            )

            # Check if Initial Balance income already exists
            existing_income = Income.query.filter_by(
                user_id=sp.user_id, type="Initial Balance", actual_date=sp.start_date
            ).first()

            if existing_income:
                print(
                    f"    ✓ Initial Balance already exists (€{existing_income.amount/100:.2f})"
                )
            else:
                ...
                if sp.initial_debit_balance > 0:
                    ...
                    print(
                        f"    + Created Initial Balance income: €{sp.initial_debit_balance/100:.2f}"
                    )
                else:
            ...
            if pre_existing_debt > 0:
                ...
                if existing_debt:
                    print(
                        f"    ✓ Pre-existing debt already exists (€{existing_debt.amount/100:.2f})"
                    )
                else:
                    ...
                    print(
                        f"    + Created Pre-existing Credit Card Debt: €{pre_existing_debt/100:.2f}"
                    )
