...
def remove_duplicate_recurring_expenses(user_id=None, dry_run=True):
    ...
    for recurring in all_recurring:
        key = (recurring.user_id, recurring.name, recurring.amount, recurring.category)
...
def remove_duplicate_salary_periods(user_id=None, dry_run=True):
    ...
    for key, periods in period_groups.items():
        if len(periods) > 1:
            ...
            print(f"  Date Range: {periods[0].start_date} to {periods[0].end_date}")
            ...
            if not dry_run:
                for period in periods[1:]:
                    ...
                    Income.query.filter_by(
                        user_id=period.user_id, budget_period_id=None
                    ).filter(
                    ...
                    Expense.query.filter_by(
                        ...
                        Expense.date.between(period.start_date, period.end_date)
                    ).delete(
                        synchronize_session=False
                    )
...
def main():
    ...
    parser.add_argument(
        "--user-id", type=int, help="Only process data for specific user ID"
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    parser.add_argument("--debts-only", action="store_true", help="Only process debts")
    parser.add_argument(
        "--recurring-only", action="store_true", help="Only process recurring expenses"
    )
    parser.add_argument(
        "--periods-only", action="store_true", help="Only process salary periods"
    )
    ...
    with app.app_context():
        ...
        if not args.debts_only and not args.periods_only:
            ...
            print("=" * 60)
            found, removed = remove_duplicate_recurring_expenses(args.user_id, dry_run)
            total_found += found
            total_removed += removed
            print(
        ...
        if not args.debts_only and not args.recurring_only:
            ...
            print("=" * 60)
            found, removed = remove_duplicate_salary_periods(args.user_id, dry_run)
            total_found += found
            total_removed += removed
            print(
