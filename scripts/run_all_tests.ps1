...
# Usage: .\scripts\run_all_tests.ps1 [suite] [-Verbose]
#
# Suite options:
...
#   .\scripts\run_all_tests.ps1 e -Verbose # E2E with verbose output
...
try {
    ...
    if ($Verbose -or $backendExitCode -ne 0) {
...
try {
    ...
    # Show output
    if ($Verbose -or $frontendExitCode -ne 0) {
...
if ($runE2E) {
    ...
    try {
        Push-Location frontend
        $e2eOutput = & npx playwright test --reporter=line 2>&1
        ...
        if ($Verbose -or $e2eExitCode -ne 0) {
