...
def validate_check_constraints():
    ...
    with app.app_context():
        ...
        if invalid_attempts:
            violations.append(
                f"✗ {len(invalid_attempts)} users with negative failed_login_attempts"
            )
        ...
        if invalid_dates:
            violations.append(
                f"✗ {len(invalid_dates)} salary periods with invalid date range"
            )
            for sp in invalid_dates:
                print(f"  - SalaryPeriod {sp.id}: {sp.start_date} >= {sp.end_date}")

        invalid_budget = SalaryPeriod.query.filter(
            SalaryPeriod.weekly_budget <= 0
        ).all()
        if invalid_budget:
            violations.append(
                f"✗ {len(invalid_budget)} salary periods with non-positive weekly_budget"
            )

        invalid_debit = SalaryPeriod.query.filter(
            SalaryPeriod.initial_debit_balance < 0
        ).all()
        if invalid_debit:
            violations.append(
                f"✗ {len(invalid_debit)} salary periods with negative initial_debit_balance"
            )

        invalid_credit_limit = SalaryPeriod.query.filter(
            SalaryPeriod.credit_limit <= 0
        ).all()
        if invalid_credit_limit:
            violations.append(
                f"✗ {len(invalid_credit_limit)} salary periods with non-positive credit_limit"
            )

        if not any(
            [invalid_dates, invalid_budget, invalid_debit, invalid_credit_limit]
        ):
        ...
        if invalid_bp_dates:
            violations.append(
                f"✗ {len(invalid_bp_dates)} budget periods with invalid date range"
            )
        ...
        if invalid_week:
            violations.append(
                f"✗ {len(invalid_week)} budget periods with week_number outside 1-4"
            )
            for bp in invalid_week:
                print(f"  - BudgetPeriod {bp.id}: week_number={bp.week_number}")

        invalid_bp_amount = BudgetPeriod.query.filter(
            BudgetPeriod.budget_amount.isnot(None), BudgetPeriod.budget_amount <= 0
        ).all()
        if invalid_bp_amount:
            violations.append(
                f"✗ {len(invalid_bp_amount)} budget periods with non-positive budget_amount"
            )
        ...
        if invalid_expenses:
            violations.append(
                f"✗ {len(invalid_expenses)} expenses with non-positive amount"
            )
        ...
        if invalid_income:
            violations.append(
                f"✗ {len(invalid_income)} income records with non-positive amount"
            )
        else:
        ...
        if invalid_debt_orig:
            violations.append(
                f"✗ {len(invalid_debt_orig)} debts with non-positive original_amount"
            )

        invalid_debt_balance = Debt.query.filter(Debt.current_balance < 0).all()
        if invalid_debt_balance:
            violations.append(
                f"✗ {len(invalid_debt_balance)} debts with negative current_balance"
            )

        invalid_debt_payment = Debt.query.filter(Debt.monthly_payment <= 0).all()
        if invalid_debt_payment:
            violations.append(
                f"✗ {len(invalid_debt_payment)} debts with non-positive monthly_payment"
            )
        ...
        invalid_rec_amount = RecurringExpense.query.filter(
            RecurringExpense.amount <= 0
        ).all()
        if invalid_rec_amount:
            violations.append(
                f"✗ {len(invalid_rec_amount)} recurring expenses with non-positive amount"
            )
        ...
        if invalid_rec_dates:
            violations.append(
                f"✗ {len(invalid_rec_dates)} recurring expenses with invalid date range"
            )
        ...
        if invalid_dom:
            violations.append(
                f"✗ {len(invalid_dom)} recurring expenses with day_of_month outside 1-31"
            )
        ...
        if invalid_dow:
            violations.append(
                f"✗ {len(invalid_dow)} recurring expenses with day_of_week outside 0-6"
            )
        ...
        invalid_cc = CreditCardSettings.query.filter(
            CreditCardSettings.credit_limit <= 0
        ).all()
        if invalid_cc:
            violations.append(
                f"✗ {len(invalid_cc)} credit card settings with non-positive credit_limit"
            )
        else:
        ...
        if invalid_ps:
            violations.append(
                f"✗ {len(invalid_ps)} period suggestions with non-positive amount"
            )
        else:
